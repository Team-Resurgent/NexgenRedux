...
void RegisterExceptionRoutines(asIScriptEngine *engine)
{
	...
	{
		r = engine->RegisterGlobalFunction("void throw(const string &in)", asFUNCTION(ScriptThrow), asCALL_GENERIC); assert(r >= 0);
		r = engine->RegisterGlobalFunction("string getExceptionInfo()", asFUNCTION(ScriptGetExceptionInfo), asCALL_GENERIC); assert(r >= 0);
	}
}
