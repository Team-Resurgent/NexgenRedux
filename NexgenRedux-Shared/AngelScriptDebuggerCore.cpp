...
string AngelScriptDebuggerCore::ToString(void *value, asUINT typeId, int expandMembers, asIScriptEngine *engine)
{
	if( value == 0 )
		return "<null>";
...
void AngelScriptDebuggerCore::RegisterToStringCallback(const asITypeInfo *ot, ToStringCallback callback)
{
...
void AngelScriptDebuggerCore::TakeCommands(asIScriptContext *ctx)
{
	for(;;)
	{
		char buf[512];
...
bool AngelScriptDebuggerCore::InterpretCommand(const string &cmd, asIScriptContext *ctx)
{
...
void AngelScriptDebuggerCore::PrintValue(const std::string &expr, asIScriptContext *ctx)
{
	if( ctx == 0 )
	{
		Output("No script is running\n");
...
void AngelScriptDebuggerCore::ListBreakPoints()
{
...
void AngelScriptDebuggerCore::ListMemberProperties(asIScriptContext *ctx)
{
	if( ctx == 0 )
	{
		Output("No script is running\n");
...
void AngelScriptDebuggerCore::ListLocalVariables(asIScriptContext *ctx)
{
	if( ctx == 0 )
	{
		Output("No script is running\n");
...
void AngelScriptDebuggerCore::ListGlobalVariables(asIScriptContext *ctx)
{
	if( ctx == 0 )
	{
		Output("No script is running\n");
...
void AngelScriptDebuggerCore::ListStatistics(asIScriptContext *ctx)
{
	if( ctx == 0 )
	{
		Output("No script is running\n");
...
void AngelScriptDebuggerCore::PrintCallstack(asIScriptContext *ctx)
{
	if( ctx == 0 )
	{
		Output("No script is running\n");
...
void AngelScriptDebuggerCore::AddFuncBreakPoint(const string &func)
{
...
void AngelScriptDebuggerCore::AddFileBreakPoint(const string &file, int lineNbr)
{
...
void AngelScriptDebuggerCore::PrintHelp()
{
	Output(" c - Continue\n"
	       " s - Step into\n"
	       " n - Next step\n"
...
void AngelScriptDebuggerCore::Output(const string &str)
{
	// By default we just output to stdout
	cout << str;
}

void AngelScriptDebuggerCore::SetEngine(asIScriptEngine *engine)
{
	if( m_engine != engine )
	{
		if( m_engine )
...
asIScriptEngine *AngelScriptDebuggerCore::GetEngine()
{
	return m_engine;
}
