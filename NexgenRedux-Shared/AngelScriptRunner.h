...
namespace NexgenRedux
{
	...
	public:
		...
		static bool ExecuteWindowMaximizeCallback(uint32_t maximized);
		static bool ExecuteWindowSizeCallback(uint32_t width, uint32_t height);
		static bool ExecuteWindowFocusCallback(uint32_t focused);
		static bool ExecuteWindowKeyboardKeyCallback(uint32_t key, uint32_t scancode, uint32_t action, uint32_t modifier);
		static bool ExecuteWindowKeyboardCharacterCallback(uint32_t codepoint);
		static bool ExecuteWindowMouseCursorPositionCallback(double xPos, double yPos);
		static bool ExecuteWindowMouseCursorEnterCallback(uint32_t entered);
		static bool ExecuteWindowMouseButtonCallback(uint32_t button, uint32_t action, uint32_t modifier);
		static bool ExecuteWindowMouseScrollCallback(double xOffset, double yOffset);
		static bool ExecuteWindowDropCallback(std::vector<std::string> paths);
